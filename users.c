...
int verificaId(stCelda enJuego[],int validos, int nuevaId)
{
    int flag=0;
    int i=0;
    ...
    while(i<validos && flag==-1)
    {
        if(enJuego[i].usr.idUsuario==nuevaId)
            flag=1;
        i++;
    }
    return flag;
...
int altaDeUsuario(stCelda enJuego[],int validos)  ///lista de jugadores
{
    ...
    do
    {
        ...
        controlNuevaId=verificaId(enJuego,validos,nuevoUsr.idUsuario);
        if(controlNuevaId==1)
            flag=1;
    }
    while(flag==1);
...
void agregaNuevoUserArchi(stUsuario nuevoUser)
{
    ...
    if(!buffArchivo)
        ...
        fwrite(&nuevoUser,sizeof(stUsuario),1,buffArchivo);

    if(fclose(buffArchivo)==-1)
        printf("\n\nERROR de Salva de Nuevo User");
}
